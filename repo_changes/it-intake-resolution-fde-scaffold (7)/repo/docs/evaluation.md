...
| Stale reads under Next.js 14 App Router | **Bug found:** `/api/metrics` served a cached `classifiedCount: 0` in ~10ms, well after real classifications existed, because Next's patched `fetch` was caching supabase-js's underlying REST GET calls independently of the route's own cache settings. | **Fixed.** Shared `lib/supabaseServer.ts` client forces `cache: "no-store"` on Supabase's internal fetch; every GET route sets `dynamic = "force-dynamic"`; `next.config.js` disables the App Router's client-side stale time. All three layers are required together. |
